...
def setup_train_args():
    ...
    # parser.add_argument('--best_model', default="dialogue_model/best_accuracy_model/", type=str, required=False, help='保存训练中准确率最高的模型路径')
...
def create_model(args, vocab_size):
    ...
    elif os.path.exists(args.dialogue_model_output_path): # 若已经进行过训练，则在原训练基础上继续训练
        model =  GPT2LMHeadModel.from_pretrained(args.dialogue_model_output_path + "model_epoch10/")
...
def train(model, device, train_list, args):
    ...
    # 记录最高的accuracy
    #_, best_accuracy  = calculate_loss_and_accuracy(model.forward(input_ids=train_dataloader)[0].to(device)), labels=train_dataloader[0], device=device)
    # best_accuracy = -1
    # 记录 out of memory的次数
    oom_time = 0
    # 开始训练
